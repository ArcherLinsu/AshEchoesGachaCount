...
function createRoleCountPage() {
    queryAllRoleData().then(rolePools => {
        ...
        // up六星不歪率
        let upSixProbability = ((rolePools.smallUpTotal / rolePools.smallTotal) * 100).toFixed(2) + '%';
              ...
              .forEach(key => {
                  if (!specialFields.includes(key)) {
                      ...
                      sixRoles.forEach(sixRole => {
                          poolTbody += `<tr><th>${sixRole.getOrder}</th><th>${sixRole.name}</th><th>${sixRole.usedNum}</th><th>${sixRole.getStatus}</th></tr>`;
                      })
                      let poolTable = `<div class="panel panel-default" id="${key}"><div class="panel-heading">${rolePools[key].name}</div><table class="table"><thead><tr><th>获得顺序</th><th>角色</th><th>所用抽数</th><th>歪歪歪?</th></tr><tbody>${poolTbody}</tbody></thead></table></div>`;
                      poolHtml += poolTable;
                  }
              })
...
function createCardCountPage() {
    queryAllCardData().then(cardPools => {
        ...
        // up六星不歪率
        let upSixProbability = ((cardPools.smallUpTotal / cardPools.smallTotal) * 100).toFixed(2) + '%';
                  ...
                  if (!specialFields.includes(key)) {
                      ...
                      if (cardPools[key].name.includes("海域同游")) {
                          ...
                          fiveCards.forEach(fiveCard => {
                              poolTbody += `<tr><th>${fiveCard.getOrder}</th><th>${fiveCard.name}</th><th>${fiveCard.usedNum}</th><th></th></tr>`;
                          })
                      } else {
                          let sixCards = cardPools[key].sixList;
                          sixCards.forEach(sixCard => {
                              poolTbody += `<tr><th>${sixCard.getOrder}</th><th>${sixCard.name}</th><th>${sixCard.usedNum}</th><th>${sixCard.getStatus}</tr>`;
                          })
                      }
                      let poolTable = `<div class="panel panel-default" id="${key}"><div class="panel-heading">${cardPools[key].name}</div><table class="table"><thead><tr><th>获得顺序</th><th>烙痕</th><th>所用抽数</th><th>歪歪歪?</th></tr><tbody>${poolTbody}</tbody></thead></table></div>`;
                      poolHtml += poolTable;
                  }
              })
        ...
        let html = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>白荆回廊抽卡统计by林苏-我们超爱白荆回廊自用版本</title><link rel="stylesheet"href="https://cdn.bootcdn.net/ajax/libs/twitter-bootstrap/3.4.1/css/bootstrap.min.css"integrity="sha384-HSMxcRTRxnN+Bdg0JdbxYKrThecOKuH5zCYotlSAcp1+c8xmyTe9GYg1l9a69psu"crossorigin="anonymous"><script src="https://code.jquery.com/jquery-3.7.1.min.js"integrity="sha256-/JqT3SQfawRcv/BIHPThkBvs0OEvtFFmqPF/lYI/Cxo="crossorigin="anonymous"></script><script src="https://cdn.bootcdn.net/ajax/libs/twitter-bootstrap/3.4.1/js/bootstrap.min.js"integrity="sha384-aJ21OjlMXNL5UyIl/XNwTMqvzeRMZH2w8c5cRVpzpU8Y5bApTppSuUkhZXN0VxHd"crossorigin="anonymous"></script></head><body><div id="main"><div class="panel panel-default"><div class="panel-heading">烙痕卡池</div><div class="panel-body"><div class="panel panel-default"><div class="panel-heading">作者说明</div><div class="panel-body"><ul><li>总抽数一项包含所有卡池抽数（常态池、up池、68元一次性卡池/友情烙痕池）。</li><li>六星/SSR数量不包含赠送的自选六星，但角色数量里包含了68元一次性卡池获取的六星。</li><li>六星/SSR平均抽计算方式：（常态池总抽数+up池总抽数）/总六星(ssr)数量(计算六星角色时会-1)。</li><li>六星(UP)/SSR(UP)平均抽计算方式：up池总抽数/up池获取的up数量。</li><li>不歪率计算方式：（up池小保底获取的up数量/up池小保底获取的总六星(ssr)数量）*100（%）; 因为大保底必出，去掉了大保底的获取数量。</li><li>常态池垫数为所有常态卡池包括常态UP卡池共用，UP池垫数为所有UP卡池共用。</li><br><li>不管怎么说，我们卡洛琳天下第一好！</li><li>本程序由天墉城海临办事处(18896499)、流月城海临办事处(26085314)、光明野海临办事处(26695651)友情支持。</li><li>本程序由监督林苏(28266118)开发，用于”我们超爱白荆回廊“群自娱自乐，不禁止二次传播，但不可用于任何商业用途；本程序为纯粹的本地浏览器脚本，不会上传任何数据至服务器，使用皆为烛龙官方开放接口，可放心使用。</li><br><li>最新脚本下载地址：https://seed.linsusu.cn</li></ul></div></div>${countHtml}${poolHtml}</div></div></div></body></html>`;
...
async function queryAllRoleData() {
    ...
    let rolePools = {
        ...
        small: true,//本次是否为小保底
        smallTotal: 0,//小保底获取的所有六星数量
        smallUpTotal: 0,//小保底获取的当期六星熟悉

        upSixTotal: 0,//up池获取的所有六星数量
        nowUpSixTotal: 0,//up池获取当期六星数量
    };
    // 垫数记录
    let usedNum = {
        // 特殊的限域巡回卡池
              ...
              for (let role of roleRecodes[key]) {
                  ...
                  if (!rolePools[role.poolId]) {
                      rolePools[role.poolId] = {
                          id: role.poolId,//卡池ID
                          name: poolInfo?.[role.poolId]?.name ?? "未知卡池", //卡池名称
                          up: up,//卡池up
                  ...
                  // 当前卡池名称
                  let nowPoolName = poolInfo?.[role.poolId]?.name ?? "未知卡池";
